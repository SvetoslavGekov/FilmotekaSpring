SELECT g.value, g.genre_id, p.product_id, p.name, p.release_year, p.pg_rating, p.duration, p.rent_cost,
	...
	AND (p.release_year IS NULL OR p.release_year BETWEEN 1990 AND 2005)
	AND (p.duration IS NULL OR p.duration BETWEEN 15 AND 300)
	AND (p.buy_cost IS NULL OR p.buy_cost BETWEEN 3 AND 20)
	AND (p.rent_cost IS NULL OR p.rent_cost BETWEEN 3 AND 20)
